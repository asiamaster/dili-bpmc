...
--创建视图
drop view ACT_ID_GROUP;
create definer=`appAccPayor`@`172.16.10.%` view ACT_ID_GROUP
as
select `role`.`id` AS `ID_`,NULL AS `REV_`,`role`.`role_name` AS `NAME_`,`role`.`firm_code` AS `TYPE_` from `role`;

drop view ACT_ID_MEMBERSHIP;
create definer=`appAccPayor`@`172.16.10.%` view ACT_ID_MEMBERSHIP
as
select `user_role`.`user_id` AS `USER_ID_`,`user_role`.`role_id` AS `GROUP_ID_` from `user_role`;

drop view ACT_ID_USER;
create definer=`appAccPayor`@`172.16.10.%` view ACT_ID_USER
as
select `user`.`id` AS `ID_`,NULL AS `REV_`,`user`.`real_name` AS `FIRST_`,NULL AS `LAST_`,`user`.`email` AS `EMAIL_`,`user`.`password` AS `PWD_`,NULL AS `PICTURE_ID_` from `user`;
